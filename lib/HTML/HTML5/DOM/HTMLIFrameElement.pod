...
HTML::HTML5::DOM::HTMLIFrameElement - implementation of the HTMLIFrameElement interface of the HTML DOM

=head1 DESCRIPTION

HTML::HTML5::DOM::HTMLIFrameElement is an implementation of the HTMLIFrameElement interface of the HTML DOM. See L<HTML::HTML5::DOM> for a list of the conventions that have been used when translating the DOM to Perl.
...
=item * C<< {http://www.w3.org/1999/xhtml}iframe >>

=back

=head2 Inheritance

HTML::HTML5::DOM::HTMLIFrameElement inherits methods from the following Perl classes.
...
Called with no arguments, is a shortcut for C<< $elem->getAttribute("sandbox") >>. Called with a defined argument, acts as C<setAttribute>. Called with undef as an argument, acts as C<removeAttribute>.

=item * C<< seamless >>

Called with no arguments, is a shortcut for C<< $elem->hasAttribute("seamless") >>. If called with a true argument, will C<setAttribute>; if called with a false argument will C<removeAttribute>.
...
Called with no arguments, is a shortcut for C<< $elem->getAttribute("srcdoc") >>. Called with a defined argument, acts as C<setAttribute>. Called with undef as an argument, acts as C<removeAttribute>.
